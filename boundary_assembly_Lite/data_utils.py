...
def iob2(tags):
    ...
    for i, tag in enumerate(tags):
        if tag == 'O':
            continue
        split = tag.split('-')
        #YPench if len(split) != 2 or split[0] not in ['I', 'B']:
...
def init_emb_weights(shape, id_to_char):
    ...
    emb_weights = Kb.variable(initializer(shape),dtype=None,name="char_embedding")
...
class BatchManager(object):
    ...
    def __init__(self, data,  batch_size):
        self.batch_data = self.sort_and_pad(data, batch_size)#将所有数据按照长短排序并按照一个批次batch_size进行分割，返回一个列表，其中的一个元素是一个批次的数据
        self.len_data = len(self.batch_data)
        self.max_sen_length = max([len(sentence[0]) for sentence in data])

    def sort_and_pad(self, data, batch_size):
        num_batch = int(math.ceil(len(data) /batch_size))#ceil(x) 函数返回一个大于或等于 x 的的最小整数
        sorted_data = sorted(data, key=lambda x: len(x[0]))#按照每个sentense的长度进行升序排序，lambda表达式，x表示data中每一个元素
        batch_data = list()
    ...
    def iter_batch(self, shuffle=False):
        if shuffle:
            random.shuffle(self.batch_data)#在批次的维度上打乱数据
