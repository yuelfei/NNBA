'''
@File name: bert_base.py
@Author: Yuefei Wu
@Version: 1.0
@Creation time: 2019/9/6 - 9:27
@Describetion: 
'''
...
def create_model(bert_config, is_training, input_ids, input_mask,segment_ids, labels, num_labels, use_one_hot_embeddings):
    ...
    :return:
    """
    # pickle.dump(input_ids, open("./pickle_file/input_ids_file","wb"))
    ...
    # 获取对应的embedding 输入数据[batch_size, seq_length, embedding_size]`[64,128,768]武乐飞
    embedding = model.get_sequence_output()

    max_seq_length = embedding.shape[1].value
    # pickle.dump(embedding,open("./pickle_file/embedding_file","wb"))
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,num_train_steps, num_warmup_steps, use_tpu,use_one_hot_embeddings):
    ...
    def model_fn(features, labels, mode, params):
        ...
        (total_loss, logits, trans, pred_ids) = create_model(bert_config, is_training, input_ids, input_mask, segment_ids, label_ids,
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(mode=mode,loss=total_loss,train_op=train_op,scaffold_fn=scaffold_fn)  # 钩子，这里用来将BERT中的参数作为我们模型的初始值
